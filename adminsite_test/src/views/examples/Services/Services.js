...
class Services extends React.Component {
  ...
  getData() {
    fetch('http://127.0.0.1:5000/table_search_service/')
  ...
  deleteServices(name_service){
    if(window.confirm('Are you sure you want to delete it?')){
      fetch('http://127.0.0.1:5000/delete_service/', {
        method:'POST',
        mode: 'cors',
        headers:new Headers({
  ...
  render() {
    const {tableData, oldServicesName } = this.state;
                  ...
                  <tr>
                    ...
                    <tbody>
                      {tableData.map((tdata,index) => (
                        <tr>
                          <td>{index+1}</td>
                          <td>{tdata.name_service}</td>
                          <td>{tdata.name_client}</td>
                          <td>{tdata.name_employee}</td>
                          <td>{tdata.note_client}</td>
                          <td>
                            <ButtonToolbar>
                              <IconButton aria-label="edit">
                                <EditIcon
                                 onClick={() => this.setState({
                                  editModalShow:true,
                                  oldServicesName : tdata.name_service
                                })}
                                />
                              </IconButton>
                              <IconButton aria-label="delete">
                                <DeleteIcon  onClick={() => this.deleteServices(tdata.name_service)}/>
                              </IconButton>
                              
                              <EditModal 
                                show = {this.state.editModalShow}
                                onHide={editModalClose}
                                oldServicesName = {oldServicesName}
                              />
                            </ButtonToolbar>
                          </td>
                        </tr>
